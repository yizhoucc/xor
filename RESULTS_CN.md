...
中文论文草稿：`docs/paper.html`。正文按摘要、引言、结果、讨论、方法和附录组织；项目状态、claim 边界与待办不再混入主文，编辑提示只保留在附录。

相关工作综述：`docs/literature_review.md`。已覆盖 25 篇直接先例、相邻方法和神经科学背景，并据此收窄 novelty：核心是宿主条件化的共享二元局部算子发现，不声称首次提出 activation search 或 discover-then-distill。

HTML 正文中的引用编号可直接跳到对应参考文献；每条参考文献均附稳定链接和一句内容及关联说明，便于写作阶段快速核对。
