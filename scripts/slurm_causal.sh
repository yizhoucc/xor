...
# Usage: sbatch scripts/slurm_causal.sh <host> <freeze> <seed> <save_dir> <inits> <host_ckpt>
HOST=${1:?}; FREEZE=${2:?}; SEED=${3:?}; SAVE_DIR=${4:?}; INITS=${5:?}; HOST_CKPT=${6:?}
...
python scripts/warmstart_causal.py --host "$HOST" --freeze "$FREEZE" --seed "$SEED" \
    --save_dir "$SAVE_DIR" --inits "$INITS" --host_checkpoint "$HOST_CKPT"
