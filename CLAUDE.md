...
### 工作笔记规则
- **`PROJECT_STATUS.md` 兼做工作笔记**：执行长任务时，把计划、执行状态、中间发现写入此文件，保持上下文和记忆。任务完成后清理为最终状态。

### 项目文档规则
- **`PROJECT_STATUS.md`**: 项目状态 + TODO + 审稿意见，统一追踪文档。**每次更新任务状态、讨论新任务、或准备新实验时，都必须同步更新此文件的 TODO 列表。**
- **`RESULTS_EN.md`** 和 **`RESULTS_CN.md`**: 实验结果正式文档，中英双语。有新结果时必须**同时更新两个文件**，保持一致。
- **`requirements.txt`** 和 **`condaenv.yml`**: 依赖管理。**每次新增 pip/conda 依赖时必须同步更新这两个文件**，确保集群环境可复现。
- **结果更新流程**: 每次拿到最新实验结果时，必须同步更新以下三个文件：
  1. `PROJECT_STATUS.md` — 更新 TODO 状态、集群任务列表、最新确认结果
  2. `RESULTS_CN.md` — 更新中文实验结果表格和总结
  3. `RESULTS_EN.md` — 更新英文实验结果表格和总结（与中文保持一致）
