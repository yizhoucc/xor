...
class LMRunner:
    ...
    def __init__(self, config):
        ...
        self.is_innernet = self.model_name in ('InnerNetLSTMModel', 'BoundedInnerNetLSTMModel', 'ClassicInnerNetLSTMModel', 'BoundedClassicInnerNetLSTMModel', 'InnerNetTransformer', 'SiLUInnerNetTransformer', 'ClassicInnerNetTransformer', 'InnerNetAttnTransformer')
        self.is_transformer = self.model_name in ('InnerNetTransformer', 'SiLUInnerNetTransformer', 'ClassicInnerNetTransformer', 'StandardTransformer', 'SwiGLUTransformer', 'InnerNetAttnTransformer')

    def _make_model(self, vocab_size):
        if self.is_transformer:
            from model.transformer import (InnerNetTransformer, SiLUInnerNetTransformer,
                                           ClassicInnerNetTransformer,
            ...
            if self.model_name == 'InnerNetTransformer':
                ...
                inner_hidden = self.config.model.get('inner_hidden', 32)
                return ClassicInnerNetTransformer(vocab_size, d_model, n_heads, d_ff,
                                                  n_layers, max_len, inner_hidden, dropout)
            ...
            from model.lstm import InnerNetLSTMModel, BoundedInnerNetLSTMModel, ClassicInnerNetLSTMModel, BoundedClassicInnerNetLSTMModel, StandardLSTMModel
            inner_hidden = self.config.model.get('inner_hidden', 32)
            ...
            elif self.model_name == 'BoundedClassicInnerNetLSTMModel':
                return BoundedClassicInnerNetLSTMModel(vocab_size, self.embed_dim, self.hidden_dim, inner_hidden)
            else:
