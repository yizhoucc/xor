...
| m4 | 计算开销分析（FLOPs + wall-clock） | ⏳ profiler脚本已实现，等待Cluster GPU运行 | `scripts/profile_compute_cost.py`将在同一GPU、同一进程中测CNN/Transformer的参数量、主要算子FLOPs、推理/训练吞吐和峰值显存，并与已有deploy吞吐交叉核对。 |
