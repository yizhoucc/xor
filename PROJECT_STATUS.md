...
### 中文 HTML 论文稿：✅ 已按正式论文结构重写

- `docs/paper.html` 现为中文单稿，删除原先英文稿和项目汇报式结构，正文顺序为摘要、引言、结果、讨论、方法、附录和参考文献。
- 主线改为：InnerNet 先作为可学习二元结构探针，在有效外围网络附近搜索局部算子；训练后读取二维表面，再把稳定结构提炼为固定算子。正文不再使用“我们不该怎么 claim”“证据等级”“论文主线”等内部汇报口吻。
- 神经科学动机已按原论文核对：两路输入类比基底树突与顶端树突，软 XOR 联系到人类皮层树突观察；正文同时说明 InnerNet 是多输入树突计算的抽象，不声称复刻完整生物神经元。
