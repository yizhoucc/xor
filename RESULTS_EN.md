...
- **Learned operators are basin-dependent** — in the complete 40-condition causal matrix, all 20 SwiGLU-host conditions converge to a scaled-SwiGLU surface (SwiGLU R²=0.985–0.991), whereas all 20 Bilinear-host conditions remain pure multiplication (mult R²=0.998–0.999). InnerNet can recover and refine a useful bivariate operator, but the operator is not a network-independent SwiGLU attractor.
...
**Comparison with the original Figure 4d:** the paper reports curves rather than exact classification values. Visual readings show that our MLP-MNIST, MLP-CIFAR-10, and CNN-MNIST results reproduce the reported scale and the learned-activation-over-ReLU direction. CNN-CIFAR-10 preserves that direction but not the original absolute level: our 2-arg/1-arg/ReLU+LN results are 78.57/81.02/75.14%, versus approximately 85.0/84.5/82.5% in the published plot. We therefore claim directional rather than exact numerical reproduction. Full protocol notes are in `docs/paper_results.md`; the comparison is visualized in `results/figures/fig_original_reproduction_comparison.{png,pdf}`.

The InnerNet panel in `results/figures/fig2_2d_activation_surfaces.{png,pdf}` is evaluated directly from the task-trained CNN checkpoint `results/figures/inner_weights_cnn_seed42.pth`; it is not a hand-constructed illustrative surface.
...
### InnerNet hidden-width ablation

| InnerNet width | PPL | Difference from width 32 | Paired-t p |
|----------------|-----|--------------------------|------------|
| 8 | 96.17±0.95 | +0.91 | 0.107 |
| 16 | 95.76±0.64 | +0.50 | 0.482 |
| 32 | **95.26±1.00** | — | — |
| 64 | **95.09±1.15** | -0.17 | 0.708 |

The omnibus Friedman test does not detect a width effect (p=0.392), and width 64 improves on width 32 by only 0.17 PPL. This supports a practical saturation claim after width 16, while the five-seed sample does not establish strict equivalence: a TOST with a pre-specified ±1 PPL margin fails for width 16 versus 32 (p=0.237). Runtime rises sharply despite negligible parameter growth: widths 8/16/64 require 10.9/15.7/53.0 GPU-hours on the same RTX 2080 Ti node. Configs: `config/experiments/transformer_wikitext_2arg_innerh{8,16,64}.yaml`; exp folders: `exp/transformer_wikitext_2arg_innerh{8,16,64}_*`; artifact: `results/audit/inner_hidden_ablation.json`.
...
A complete causal host matrix gives the complementary result. Under a Bilinear-GLU host, all 20 conditions (joint/frozen × random/multiply × 5 seeds) are classified as multiplication: joint random/multiply reach PPL **73.70±0.59 / 73.79±0.45** with mult R² **0.9976 / 0.9989**, while frozen controls remain near host performance (PPL **79.57±0.73 / 79.56±0.72**) with mult R² **0.9993 / 0.9990**. Under a SwiGLU host, all 20 conditions (4 initializations × 5 seeds) are classified as SwiGLU, with PPL **71.98–72.21** and SwiGLU R² **0.9853–0.9912**. Operator identity is therefore determined by the host/optimization basin rather than InnerNet initialization, rejecting a universal network-independent SwiGLU attractor. PPL gains cannot be assigned solely to activation replacement because continued-host controls were not run.
...
Config: `scripts/distill_crossinit.py`, `scripts/warmstart_causal.py`, and `scripts/distill_innernet.py`. Machine-readable results: `results/audit/causal_matrix_summary.json` and `causal_matrix_conditions.csv`.
...
All structured local experiment artifacts are indexed by a canonical manifest (`scripts/build_result_manifest.py`) and aggregated by scientific configuration, condition, and run outcome rather than directory name (`scripts/summarize_result_manifest.py`). The audit includes unified-runner artifacts and standalone deploy, Sequential-MNIST, warm-start, and PPO results: 1,708 metric rows from 510 experiment directories, with 441 raw-verified and 69 incomplete experiments. It produces 306 reportable groups with raw seed values, both sample and population standard deviations, and no unresolved within-configuration seed conflicts. PPO runs include a per-seed mean over the final 20 recorded evaluation points, preventing a single seed's terminal log value from being mistaken for the cross-seed result. Failed/NaN runs remain explicit rather than being silently removed; for example, the constrained Sequential-MNIST model has an eight-seed successful group at 98.435% (sample SD 0.236%; population SD 0.220%) and a separately retained NaN run. A single reused experiment name (`mlp_mnist_relu`) was detected and separated into its unmatched 64-width and parameter-matched 112-width configurations.

Thirty-two headline comparisons are registered in `config/audit/core_comparisons.yaml` and regenerated as `results/audit/core_comparisons.csv`. For InnerNet versus GELU at d=64/128/192/256, paired-t p-values are 0.00022/0.05095/0.361/0.173; SwiGLU versus InnerNet at d=128 gives p=0.00917. The hidden-width comparisons against h=32 give p=0.107/0.482/0.708 for h=8/16/64. New scalar-baseline tests give p=0.0056 for InnerNet versus PReLU and p=0.00052 versus Swish. FashionMNIST 1-arg versus 2-arg gives p=0.0504, whereas SVHN 2-arg versus 1-arg gives p=0.374. Because a two-sided Wilcoxon test with n=5 has coarse resolution (typically a minimum p=0.0625), formal reporting includes raw seeds, bootstrap confidence intervals, parametric and non-parametric tests, and Cohen's dz rather than relying on a single threshold.

The registered headline cells in RESULTS_CN/EN are also checked directly against the canonical summary; all 76 currently registered cells match. Audit artifacts: `results/audit/grouped_metric_summary.csv`, `metric_conflicts.csv`, `experiment_variant_collisions.csv`, `core_comparisons.csv`, and `document_consistency.csv`.
...
An additional same-process FP32 profile on one RTX 2080 Ti separates arithmetic count from implementation overhead. Relative to SwiGLU, InnerNet has only **1.17×** profiled major-op FLOPs, yet is **17.33×/12.02× slower at inference** and **7.63×/6.27× slower for a training step** in the CNN/Transformer benchmarks, with **7.90×/6.43×** peak training memory. The gap is therefore dominated by unfused per-element MLP execution and intermediate materialization, not parameter count or nominal FLOPs. The unfused polynomial distillation improves inference by 4.84×/4.37× over InnerNet, but remains 3.58×/2.75× slower than SwiGLU. Source: `results/audit/compute_cost_profile.json`.
